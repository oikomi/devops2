...
echo "正在安装MySQL 8 软件....... form https://github.com/hcymysql/mysql_install"
...
######mgr配置######
mysql_port=3306
primary_ip=10.189.108.73
secondary1_ip=10.189.108.74
secondary2_ip=10.189.108.75

primary_port=33061
secondary1_port=33062
secondary2_port=33063

local_ip=10.189.108.73
local_port=33061
...
sed "s/test/$dbname/g;s/3306/$dbport/;s/413306/$serverId/;/innodb_buffer_pool_size/s/1/$innodb_bp_size/" my_test.cnf > /etc/my.cnf
...
then
	...
	echo "初始化MySQL数据目录......"
	echo ""
	bin/mysqld --defaults-file=/etc/my.cnf --initialize --user=mysql --basedir=/usr/local/mysql --datadir=/data/mysql/$dbname/data
	sleep 5
...
echo "" > /usr/lib/systemd/system/mysql.service
...
echo "random_passwd is ${random_passwd}"
echo "root_passwd is ${root_passwd}"
